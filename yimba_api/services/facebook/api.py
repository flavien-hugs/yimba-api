...
@router.get(
    "/{keyword}/statistics",
    dependencies=[Security(AuthTokenBearer(allowed_role=["admin", "client"]))],
    response_model=model.FacebookStatistic,
...
async def statistic(keyword: str):
    try:
        ...
        facebook_data = model.FacebookInDB.find(router.storage, search_filter)
        facebook_data_count = await model.FacebookInDB.count(
        ...
        result = model.FacebookStatistic(**totals)
