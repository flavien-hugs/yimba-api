...
from yimba_api import PyObjectId

disable_installed_extensions_check()
CustomPage = Page.with_custom_options(size=Field(10, ge=1))
...
async def get(storage, model, oid: str, name: str = None):
    if (result := await model.find_one(storage, {"_id": oid})) is not None:
        return result
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND, detail=f"{name or 'Item'} not found."
    )
